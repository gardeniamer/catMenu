		...
		methods: {
			...
			pet() {
				...
				const catGet = new Promise((resolve, reject)=>{
					uni.request({
						method:"POST",
						url:'https://api.shuzike.com/lifeservice/check2/103',
						header:{
						...
						data:{
								...
								petType: String(uni.getStorageSync("petKind")),
								recogType: "1",
								groupId:""
							},
						success: (res2) => {
							if (res2.data.msg == "成功") {
								const petId = res2.data.data.result.petIds[0]
								if(uni.getStorageSync("petMsg")[petId] ) {
									const data = uni.getStorageSync("petMsg")[petId] 
									...
									reject("数据加载失败，请联系管理员")
								}
							}
							else {
								reject(res2.data.msg)
							}
						}
					})
				})
